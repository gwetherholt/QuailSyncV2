"""Roboflow frame upload over the REST upload API.

Adapted from ``incubator/roboflow_uploader.py`` (REST, ``requests`` — no heavy
SDK). :meth:`RoboflowUploader.upload_frame` always uploads the full frame; when
it is *given* ``detections`` it additionally POSTs a YOLO-format pre-annotation
(+ labelmap) for them as reviewable pre-labels.

Whether detections are supplied is the caller's decision, and that is how the
per-project annotation policy is enforced: the service loop passes detections
only for the chick model (project ``find-chicks-5``) and passes **none** for the
incubation model, so ``incubation-stages`` receives raw images with no
annotations (matching the ``incubator/`` pipeline). The target **project** comes
from the current mode's config; the service loop sets
:attr:`RoboflowUploader.project` on a model swap. Workspace, API key and batch
name are shared.
...
class RoboflowUploader:
    """Uploads frames to a Roboflow project (REST), optionally with YOLO pre-labels.
    ...
    def upload_frame(self, frame, name: str, detections=None, *, cv2_module=None) -> bool:
        """Upload ``frame`` as ``name``; if ``detections`` are supplied, also POST
        a YOLO pre-annotation (+ labelmap) for them.

        Returns True when the image upload succeeds (annotation is best-effort on
        top). Never raises. With **no** ``detections`` the frame is uploaded
        image-only — which is how the caller keeps incubation uploads
        annotation-free (it passes detections only for the chick model).
        """
