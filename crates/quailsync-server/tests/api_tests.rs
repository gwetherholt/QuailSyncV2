...
mod lineage_tests {
    ...
    /// with a truncated -- or empty -- lineage set.
    ///
    /// Forcing mechanism: `bird_lineages.lineage_id REFERENCES lineages(id)`
    /// with `PRAGMA foreign_keys = ON`, so a nonexistent id fails the INSERT
    /// after the DELETE has already run.
    #[tokio::test]
    async fn put_bird_lineages_rolls_back_on_bad_id() {
        let base = spawn_test_server().await;
        let client = reqwest::Client::new();
        let (a, b) = seed_two_lineages(&base, &client).await;
        let bird_id = seed_bird(&base, &client, vec![a.id, b.id], Sex::Female).await;

        // Second id does not exist -> the INSERT fails after the DELETE.
        let resp = client
            .put(format!("{base}/api/birds/{bird_id}/lineages"))
            .json(&ReplaceLineagesRequest {
        ...
        // all not emptied.
        let after: Bird = reqwest::get(format!("{base}/api/birds/{bird_id}"))
            .await
            .unwrap()
            .json()
            .await
