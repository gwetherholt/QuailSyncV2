"""Frame grabbing for the incubator pipeline.
...
detectors, and tests all talk to the protocol, so a test passes a
...
frame, and returns the freshest one. Re-opening per grab is fine at the
incubator's slow cadence (~1 frame / 10 s) and keeps the credentialed URL in
this process's memory — never on a command line, so it can't leak via ``ps``.
"""
...
logger = logging.getLogger("incubator.camera")
    ...
    def __init__(self, source: str, *, warmup_frames: int = 3, cv2_module=None):
        if not source:
            raise CaptureError(
                ...
                "camera.source_env (e.g. INCUBATOR_RTSP_URL) in the incubator secrets file"
            )
        self.source = source
...
def create_frame_source(conf, *, cv2_module=None) -> FrameSource:
    ...
    if not source:
        raise CaptureError(
            ...
            "(the incubator secrets file provides it to the systemd unit)"
        )
